...
export const fullScoreInfo: readonly ScoreInfo[] = scoreInfoJson as any;
export const getScoreInfo = () => {
  return [...fullScoreInfo];
};
...
export type FingerType = "fingerNone" | "fingerThreeValves" | "fingerTrombone";
  ...
  for (const element of measures) {
    ...
    for (const element of elements) {
      ...
      if (fingering !== null) {
        ...
        if (val !== undefined && fingering !== "fingerNone") {
          if (fingering === "fingerTrombone") {
            val = toTrombone[val];
          }
